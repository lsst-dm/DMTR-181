...
for comcam or MTcam. ~ There is no need to separate the forwarders and
archiver systems from the DAQ at a long distance.~ ~

\subsection{Recommended Improvements}
\label{sect:recommendations}

No improvements other than the MTcam will not use the archiver system,
and the image will be taken directly from the DAQ. ~ In the future the
DAQ needs to now build fits files, so there is no reason to have the
archiver fits files AND DAQ fits files. ~ We will in the future cut out
the archiver function's of build of the fits files. ~\\
\hspace*{0.333em}
